...
class App extends Component {
  ...
  addNewBucket = (bucket) => {
    ...
    table.row
      .add(
        $(
          `<tr id = ${bucket.id} key=${bucket.id}><td> ${bucket.id} </td> <td> ${bucket.bucketName}</td> <td className="row"> <div className="col-10 my-auto">${bucket.lastUpdate}</div> <div className="col-2 my-auto"></div></td></tr>`
        )
      )
      .draw();
  };
