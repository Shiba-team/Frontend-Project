...
class ListBucket extends Component {
  ...
  deleteRow = id => {
    let del = this.state.deleteButton;
    del.push(id);
    this.setState({
      deleteButton: del,
    })
  ...
  addDeleteButton = (id) =>{
    ...
    if (this.state.deleteButton.indexOf(id)>-1)
    return (
      <button type="button" className="btn btn-danger disabled" onClick={()=> this.deleteRow(id)}><i class="fa fa-trash"></i></button>)
    else return (
      <button type="button" className="btn btn-danger disabled" onClick={()=> this.deleteRow(id)}><i class="fa fa-trash"></i></button>)
    else return;
  }
