...
class ListBucket extends Component {
  ...
  deleteRow = id => {
    ...
    for (let i = 0; i < this.props.buckets; i++){
      if (deleteButton.indexOf(buckets[i].id) > -1)
        $(`#button_${buckets[i].id}`).addClass('disabled');
        else $(`#button_${buckets[i].id}`).removeClass('disabled');
    }
  }
  ...
  addDeleteButton = (id) =>{
    ...
    return (
      <button id={`button_`+id} type="button" className={buttonClass} onClick={()=> this.deleteRow(id)}><i class="fa fa-trash"></i></button>)
    else return;
  }
