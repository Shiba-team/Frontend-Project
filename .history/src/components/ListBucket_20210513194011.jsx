...
class ListBucket extends Component {
  ...
  addDeleteButton = (id) =>{
    ...
    return (
      <button type="button" className={buttonClass} onClick={()=> this.deleteRow(id)}><i class="fa fa-trash"></i></button>)
    else return;
  }
  ...
  render() {
    function delRow (data, id ) {
      let filteredRows = data.rows.filter(row => row !== data.rows[data.indexOf(id)]);
    ...
    let buckets = this.props.buckets;
      for (let i = 0; i < buckets.length; i++){
        let bucket = {
          ...
          clickEvent: () => delRow(listBucket, buckets[i].id)
        }
