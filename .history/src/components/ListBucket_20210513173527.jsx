...
class ListBucket extends Component {
  ...
  addDeleteButton = (id) =>{
    ...
    return (
      <button type="button" className="btn btn-danger" onClick={(event)=> {event.target.classList.add('disabled');this.deleteRow(id)}}><i class="fa fa-trash" onClick={(event=>event.target.classList.add('disabled'))}></i></button>)
    else return;
  }
