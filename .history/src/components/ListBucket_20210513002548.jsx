...
class ListBucket extends Component {
  ...
  buildRow = (bucket) =>{
    ...
    return (
      ...
      <td> <div className="row"><div className="col-10 my-auto" >{bucket.lastUpdate}</div> <div className="col-2 my-auto">{this.addDeleteButton(bucket.id)}</div></div></td>
      </tr>
    )
  }
  ...
  render() {
    ...
    for (let i = 0; i < this.state.buckets.length; i++){
      let bucket = {
        ...
        lastUpdate: <div className="row"><div className="col-10 my-auto" >{bucket.lastUpdate}</div> <div className="col-2 my-auto">{this.addDeleteButton(this.state.buckets[i].id)}</div></div>,
      }
