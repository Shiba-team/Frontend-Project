...
class ListBucket extends Component {
  ...
  deleteRow = id => {
    setDeleteButton(deleteButton.push(id));
  ...
  addDeleteButton = (id) =>{
    ...
    if (deleteButton.indexOf(id)>-1)
    return (
      <button type="button" className="btn btn-danger disabled" onClick={()=> setDeleteButton(this.deleteRow(id))}><i class="fa fa-trash"></i></button>)
    else return (
